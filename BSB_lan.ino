...
// list of enabled modules
          client.println(F("<BR>" MENU_TEXT_MOD ": "));
          outBufclear();
          #ifdef WEBSERVER
...
#if defined LOGGER || defined WEBSERVER
          strcpy_P(outBuf, PSTR("<BR><BR>free space: %lu MB<br>free clusters: %lu<BR>freeClusterCount() call time: %lu microseconds<BR><br>\n"));
          ...
          bufferedprint(PSTR("<BR><BR>" MENU_TEXT_AVT ": <BR>"));
          for (int i=0; i<numAverages; i++) {
            if (avg_parameters[i] > 0) {
         ...
         //bufferedprint(br_html);
          ...
          #ifdef LOGGER
          bufferedprint(PSTR("<BR><BR>" MENU_TEXT_LGP " "));
...
// use the sub-topic "json" if json output is enabled
          ...
          if (log_parameters[i] >= 20100 && log_parameters[i] < 20200) {
            ...
            if (hum > 0 && hum < 101) {
              int len = _printFIXPOINT(smallbuf, temp, 2);
              strcat_P(smallbuf, PSTR(" / "));
              _printFIXPOINT(smallbuf+len, hum, 2);
              MQTTClient.publish(MQTTTopic.c_str(), smallbuf);
            }
#endif
          }
